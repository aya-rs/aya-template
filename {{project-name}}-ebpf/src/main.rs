...
{% case program_type -%}
{%- when "kprobe" %}
use aya_ebpf::{cty::c_long, macros::kprobe, programs::ProbeContext};
...
{%- when "kretprobe" %}
use aya_ebpf::{cty::c_long, macros::kretprobe, programs::RetProbeContext};
use aya_log_ebpf::info;
...
fn try_{{crate_name}}(ctx: RetProbeContext) -> Result<(), c_long> {
    info!(&ctx, "function {{kprobe}} called");
    Ok(())
}
{%- when "fentry" %}
use aya_ebpf::{cty::c_long, macros::fentry, programs::FEntryContext};
use aya_log_ebpf::info;
...
{%- when "fexit" %}
use aya_ebpf::{cty::c_long, macros::fexit, programs::FExitContext};
use aya_log_ebpf::info;
...
{%- when "uprobe" %}
use aya_ebpf::{cty::c_long, macros::uprobe, programs::ProbeContext};
use aya_log_ebpf::info;
...
{%- when "uretprobe" %}
use aya_ebpf::{cty::c_long, macros::uretprobe, programs::RetProbeContext};
use aya_log_ebpf::info;
...
{%- when "sock_ops" %}
use aya_ebpf::{cty::c_long, macros::sock_ops, programs::SockOpsContext};
use aya_log_ebpf::info;
...
{%- when "xdp" %}
use aya_ebpf::{bindings::xdp_action, cty::c_long, macros::xdp, programs::XdpContext};
use aya_log_ebpf::info;
...
{%- when "classifier" %}
use aya_ebpf::{bindings::TC_ACT_PIPE, cty::c_long, macros::classifier, programs::TcContext};
use aya_log_ebpf::info;
...
{%- when "cgroup_skb" %}
use aya_ebpf::{cty::c_long, macros::cgroup_skb, programs::SkBuffContext};
use aya_log_ebpf::info;
...
{%- when "lsm" %}
use aya_ebpf::{cty::c_long, macros::lsm, programs::LsmContext};
use aya_log_ebpf::info;
...
{%- when "tp_btf" %}
use aya_ebpf::{cty::c_long, macros::btf_tracepoint, programs::BtfTracePointContext};
use aya_log_ebpf::info;
...
{%- when "socket_filter" %}
use aya_ebpf::{cty::c_long, macros::socket_filter, programs::SkBuffContext};
...
{%- when "cgroup_sysctl" %}
use aya_ebpf::{cty::c_long, macros::cgroup_sysctl, programs::SysctlContext};
use aya_log_ebpf::info;
...
{%- when "cgroup_sockopt" %}
use aya_ebpf::{cty::c_long, macros::cgroup_sockopt, programs::SockoptContext};
use aya_log_ebpf::info;
...
{%- when "raw_tracepoint" %}
use aya_ebpf::{cty::c_long, macros::raw_tracepoint, programs::RawTracePointContext};
use aya_log_ebpf::info;
...
{%- when "perf_event" %}
use aya_ebpf::{
    cty::c_long, helpers::bpf_get_smp_processor_id, macros::perf_event, programs::PerfEventContext,
    EbpfContext,
};
use aya_log_ebpf::info;

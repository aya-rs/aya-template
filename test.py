        ...
        if platform.system() == "Linux":
            ...
            try:
                ...
                for i, line in enumerate(process.stdout):
                    ...
                    if b"Waiting for Ctrl-C" in line:
                        sys.stdout.write("Sending EXT\n")
                        sys.stdout.buffer.flush()
                        process.stdin.write(b"\x03")
                        process.stdin.flush()
                        time.sleep(5)
                        sys.stdout.write("Sending SIGINT\n")
                        sys.stdout.buffer.flush()
                        process.send_signal(signal.SIGINT)
                    else:
                        sys.stdout.write("Got:\n")
                        sys.stdout.write(line.hex())
                        sys.stdout.write("\n")
                        sys.stdout.write("Expected:\n")
                        sys.stdout.write(b"Waiting for Ctrl-C".hex())
                        sys.stdout.flush()
            finally:
